...
//////////////////////////////////////////////////////////////////////       SPRINT         //////////////////////////////////////////
	...
	//if (forward > 0.f) Player_Forward(player, movement);

	bool releasedSprint = WasKeyReleased(ctrl->controlScheme.forward, &agnosticInput);
	if (ctrl->sprintTimer >= 0.f) {
		if (forward > 0.f) player->sprinting ^= true;
		ctrl->sprintTimer += dt;
		if (ctrl->sprintTimer > 0.25f) ctrl->sprintTimer = -1.f;
	} else if (releasedSprint) {
		ctrl->sprintTimer = 0.f;
	}
	if (releasedSprint) {
