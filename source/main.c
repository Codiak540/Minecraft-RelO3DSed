...
//        SpriteBatch_PushQuad(35, 0, 0, 128, 128, 0, 0, 256, 256);
	...
	while (aptMainLoop()) {
		...
		static bool returntogame = false;
		static bool returntomenu = false;
		hidScanInput();
		u32 keysheld = hidKeysHeld(), keysdown = hidKeysDown();
		if (keysdown & KEY_DLEFT && invtab > 1) 
			invtab -= 1;
		if (keysdown & KEY_DRIGHT && invtab < 2)
			invtab += 1;
		if (invtab == 1){
			PlayerInv1(&player);
		}
		...
		if (keysdown & KEY_START) {
			...
			if (gamestate == GameState_Playing) {
				...
				returntogame = Gui_Button(0.5f, "Return");
				returntomenu = Gui_Button(0.5f, "Exit");
				Gui_EndRow();
